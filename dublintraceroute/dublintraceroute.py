...
class DublinTraceroute(_dublintraceroute.DublinTraceroute):
    def __str__(self):
        return ('<{self.__class__.__name__} (target={target!r}, '
                ...
                'broken_nat={broken_nat!r}, use_srcport_for_path_generation={use_srcport_for_path_generation!r})>'.format(
                    self=self,
                    target=self.target,
                    sport=self.sport,
                    dport=self.dport,
...
def probe(target, sport=None, dport=None, npaths=1, ttl=64, delay=None,
    ...
    d = DublinTraceroute(target, sport, dport, npaths, min_ttl=ttl, max_ttl=ttl,
                         delay=delay, broken_nat=broken_nat, use_srcport_for_path_generation=use_srcport_for_path_generation)
    result = d.traceroute()
    ret = []
